...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildCropTickerCapsuleHtml(crop) {
    ...
    return `
      <div class="msp-ticker-item" data-crop="${safeCropName}" role="button" tabindex="0" title="Click to view full ${safeCropName} MSP schedule and historical rates">
  ...
  function renderFarmerMspTicker(season = 'all') {
    ...
    const capsulesHtml = items.map(buildCropTickerCapsuleHtml).join('');

    // Duplicate track for 100% seamless, infinite continuous roll
    track.innerHTML = `
      <div class="msp-ticker-loop" style="display:inline-flex; align-items:center; gap:14px;">${capsulesHtml}</div>
      <div class="msp-ticker-loop" aria-hidden="true" style="display:inline-flex; align-items:center; gap:14px;">${capsulesHtml}</div>
    `;
  ...
  const FARMER_TRANSLATIONS = {
    ...
    hi: {
      ...
      mod6Desc: "प्रत्यक्ष लाभ अंतरण (डीबीटी) बैंक �      tickerTitle: "തത്സമയ MSP 2026-27",
  ...
  function applyFarmerLanguage(lang, notify = true) {��கவும்.",
